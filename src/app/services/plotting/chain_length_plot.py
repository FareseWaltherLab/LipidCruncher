...
Creates bubble charts showing the distribution of total carbon chain lengths
and double bonds per lipid class, with bubble size proportional to mean
concentration.
...
from app.lipid_nomenclature import HYDROXYL_PATTERN, parse_lipid_name
...
class ChainLengthPlotterService:
    ...
    def parse_total_chain_info(lipid_name: str) -> Optional[Tuple[int, int]]:
        ...
        try:
            ...
            for part in parts:
                # Strip hydroxyl notation (e.g. '18:1;O2' → '18:1')
                cleaned = HYDROXYL_PATTERN.sub('', part)
                if ':' not in cleaned:
                    continue
                carbon_str, db_str = cleaned.split(':', 1)
                total_carbons += int(carbon_str)
                total_double_bonds += int(db_str)
    ...
    @staticmethod
    def create_chain_length_figure(
        data: ChainLengthData,
        color_mapping: Dict[str, str],
    ) -> go.Figure:
        """Create a 2-panel bubble chart: carbon chain length and double bonds.

        Left panel: x = lipid class, y = total carbon chain length.
        Right panel: x = lipid class, y = total double bonds.
        Bubble size is proportional to mean concentration.

        Args:
            data: ChainLengthData from calculate_chain_length_data().
        ...
        # Compute marker sizes scaled by concentration
        all_conc = rec_df['MeanConcentration'].values
        size_arr = _scale_marker_sizes(all_conc)
        ...
        for i, row in rec_df.iterrows():
            ...
            shared = dict(
                ...
                hovertemplate=(
                    f"<b>{cls}</b><br>"
                    f"Carbons: {int(row['TotalCarbons'])}<br>"
                    f"Double bonds: {int(row['TotalDoubleBonds'])}<br>"
                    f"Mean concentration: {row['MeanConcentration']:.2f}"
                    "<extra></extra>"
                ),
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=[cls], y=[row['TotalCarbons']], **shared,
                ),
                row=1, col=1,
            )

            # Right panel: double bonds (hide duplicate legend entry)
            fig.add_trace(
                go.Scatter(
                    x=[cls], y=[row['TotalDoubleBonds']],
        ...
        fig.update_layout(
            ...
            width=CHART_WIDTH,
            plot_bgcolor='white',
            legend=dict(title='Lipid Class'),
        )
        fig.update_xaxes(
            title_text='Lipid Class', showgrid=False, row=1, col=1,
        )
        fig.update_xaxes(
            title_text='Lipid Class', showgrid=False, row=1, col=2,
        )
        fig.update_yaxes(
        ...
        fig.update_yaxes(
            ...
            gridcolor='lightgray', row=1, col=2,
        )
...
def _scale_marker_sizes(values: np.ndarray) -> np.ndarray:
    ...
    v_min, v_max = sqrt_vals.min(), sqrt_vals.max()
    if v_max == v_min:
