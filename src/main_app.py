...
def process_experiment(df, data_format='lipidsearch'):
    ...
    # Validate inputs before creating ExperimentConfig
    if not all(label.strip() for label in conditions_list):
        st.sidebar.error("All condition labels must be non-empty.")
        return False, None, None, None, False, None

    if not all(count > 0 for count in number_of_samples_list):
        st.sidebar.error("All sample counts must be greater than 0.")
        return False, None, None, None, False, None

    # Create experiment configuration
    try:
        experiment = ExperimentConfig.from_user_input(
        ...
        st.sidebar.error(f"Invalid experiment setup: {str(e)}")
        return False, None, None, None, False, None

    # NOW process group samples (this creates group_df)
        ...
        with upload_container:
            ...
            if uploaded_file is not None:
                try:
                    ...
                    new_standards_df = StandardsService.process_standards_file(
                        standards_df,
...
def handle_data_normalization(cleaned_df, intsta_df, experiment, format_type):
    """
    Handle data normalization using refactored workflow.
    ...
    return workflow.run(cleaned_df, intsta_df, experiment, format_type)
