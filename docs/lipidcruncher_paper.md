...
LipidCruncher offers two types of heatmaps. Fatty acid composition heatmaps [22] display lipids within a selected class by double bond count (x-axis) and carbon chain length (y-axis). Color intensity indicates each species' relative abundance within the class (Supplementary Methods S6.4). Multiple conditions are shown side-by-side with average double bond and chain length markers overlaid, enabling detection of compositional shifts. Comprehensive lipidomic heatmaps display all lipids (rows) across samples (columns) with z-score color coding. Users can preserve original lipid order or apply Ward’s hierarchical clustering [23] to group lipids with similar concentration patterns (Supplementary Methods S6.1-S6.2).
